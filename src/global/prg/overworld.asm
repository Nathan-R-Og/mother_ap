...
OpenPresent:
    ...
    sta soundqueue_pulseg0
    ldy #$06
    jsr archipelago_give_item_present
    nop
    nop
    nop
    bne B19_0a05
...
;literally just to override $ff. its checked through flags anyways
archipelago_give_item_present:
    lda (object_data), y
    cmp #$80
    bcs @give_nothing
    sta curr_item_id
...
archipelago_give_item:
    ;if item == $ff, dont actually give to player
    lda curr_item_id
    cmp #$ff
    beq archipelago_dump_object
    ;else :)
    sta (temp_vars), y
archipelago_dump_object:
    jsr EnablePRGRam
    ;0 - key
    ldx #0
    lda #$39
...
;return carry clear if succeed
ap_fill_current:
    ;store to picked item
    sta curr_item_id
    jsr ap_fill_first_empty_inventory
    lda #0
    sta curr_item_id
    rts

ap_get_first_empty_inventory:
    lda #0
    sta curr_item_id
;return carry set if fail
;return carry clear if succeed
ap_fill_first_empty_inventory:
    ;fill inventory
    ldx #0
    @all_loop:
    jsr GetXCharacter
